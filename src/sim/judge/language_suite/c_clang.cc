...
C_Clang::C_Clang(Standard standard)
...
sandbox::Result C_Clang::run_compiler(
...
sandbox::Result C_Clang::is_supported_impl(const CompileOptions& options) {
    return run_compiler({{"--version"}}, std::nullopt, {}, options);
}

sandbox::Result C_Clang::compile_impl(
    FilePath source, FilePath executable, int compilation_errors_fd, const CompileOptions& options
) {
    return run_compiler(
        {{"source.c", "-o", "exe"}},
