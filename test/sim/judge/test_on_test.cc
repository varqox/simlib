...
void compile(Suite& suite, StringView source) {
    auto source_file = TemporaryFile{"/tmp/sim_judge_test_on_test_prog.XXXXXX"};
...
TEST(test_on_test, prog_tle) {
    ...
    EXPECT_EQ(report.comment, "Time limit exceeded");
...
TEST(test_on_test, prog_mle) {
    ...
    EXPECT_EQ(report.comment, "Memory limit exceeded");
    EXPECT_EQ(report.score, 0);
}
...
TEST(test_on_test, prog_output_size_limit_exceeded) {
    ...
    EXPECT_EQ(report.comment, "Output size limit exceeded");
    EXPECT_EQ(report.score, 0);
}
...
// NOLINTNEXTLINE
TEST(test_on_test, test_io_simple) {
...
// NOLINTNEXTLINE
TEST(test_on_test, test_io_is_not_seekable) {
...
// NOLINTNEXTLINE
TEST(test_on_test, test_io_closes_partially_read_stdin_before_writing_to_stdout) {
    auto test_inout = TemporaryFile{"/tmp/sim_judge_test_on_test_inout.XXXXXX"};
    put_file_contents(
        test_inout.path(),
...
// NOLINTNEXTLINE
TEST(test_on_test, test_io_closes_stdout_and_reads_stdin_afterwards) {
    auto test_inout = TemporaryFile{"/tmp/sim_judge_test_on_test_inout.XXXXXX"};
    put_file_contents(
        test_inout.path(),
        from_unsafe{concat_tostr(std::string(3e5, 'a'), '\n', std::string(3e5, 'a'), '\n')}
